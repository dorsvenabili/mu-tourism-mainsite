...
if ( !is_main_site() ) {
	wp_redirect( network_home_url( 'crea-tu-blog' ) );
	die();
}
...
function signup_another_blog($blogname = '', $blog_title = '', $errors = '') {
	...
	<form id="setupform" method="post" action="<?php echo get_bloginfo('home').'/crea-tu-blog/'; ?>">
...
function validate_blog_signup() {
	...
	mecus_wpmu_signup_blog($domain, $path, $blog_title, $user_name, $user_email, $meta);
	mecus_confirm_blog_signup($domain, $path, $blog_title, $user_name, $user_email, $meta);
	return true;
}

function mecus_confirm_blog_signup($domain, $path, $blog_title, $user_name = '', $user_email = '', $meta) {
	?>
	<h2><?php printf( __( '¡Felicidades! Tu nuevo sitio , %s, ya está casi listo.' ), "<a href='http://{$domain}{$path}'>{$blog_title}</a>" ) ?></h2>

	<p><?php _e( 'Pero, antes de que puedas comenzar a usar tu sitio, <strong>debes confirmar tu petición</strong>.' ) ?></p>
	<p><?php printf( __( 'Check your inbox at <strong>%s</strong> and click the link given.' ),  $user_email) ?></p>
	<p><?php _e( 'Si no confirmas tu sitio en dos días, tendrás que registrarlo de nuevo.' ); ?></p>
	<p><?php _e( 'Una vez que hayas confirmado tu solicitud de registro de nuevo blog, el administrador te mandará un email con tus nuevos datos de acceso.' ); ?></p>
...
//Funciones necesarias para la página crea-tu-blog.php
...
function mecus_wpmu_signup_blog($domain, $path, $title, $user, $user_email, $meta = '') {
	global $wpdb;
	...
	mecus_confirmation_user_email($domain, $path, $title, $user, $user_email, $key, $meta);
	
}

function mecus_confirmation_user_email($domain, $path, $title, $user, $user_email, $key, $meta = ''){
	global $current_site;

	if ( !apply_filters('mecus_confirmation_user_email', $domain, $path, $title, $user, $user_email, $key, $meta) )
		return false;
	
	
	$user_email_parts = explode('@',$user_email);
	$user_email_part_1 = $user_email_parts[0];
	$user_email_parts_2 = explode('.',$user_email_parts[1]);
	$user_email_part_2 = $user_email_parts_2[0];
	$user_email_part_3 = $user_email_parts_2[1];
	
	
	// Send email with activation link.
	if ( !is_subdomain_install() || $current_site->id != 1 )
		$confirmation_url = "http://{$domain}/confirmacion/?key=$key"; // @todo use *_url() API
	else
		$confirmation_url = "http://{$domain}/confirmacion/?key=$key"; // @todo use *_url() API

	

	$confirmation_url = esc_url($confirmation_url);
	...
	$message = sprintf( apply_filters( 'wpmu_signup_blog_notification_email', __( "Para confirmar que has hecho una petición de creación de blog, por favor haz click en el siguiente enlace:\n\n%s\n\nEn los próximos días recibirás la confirmación del administrador y un email con tus datos de acceso a tu nuevo blog.\n\nSi no confirmas tu petición haciendo click en el enlace, tu petición no será tenida en cuenta y no se creará dicho blog, así que si no tienes intención de usarlo no lo confirmes por favor.\n\nUn saludo.\n\n Equipo de Blogs de Turismo de la Comunitat Valenciana" ) ), $confirmation_url, $key );
	// TODO: Don't hard code activation link.
	
	$subject = sprintf( apply_filters( 'wpmu_signup_blog_notification_subject', __( '[%1s] Confirmar %2s' ) ), $from_name, esc_url( 'http://' . $domain . $path ) );
...
if ( $active_signup == "none" ) {
	...
	if ( is_ssl() )
		...
		$proto = 'http://';
	$login_url = site_url( 'wp-login.php?redirect_to=' . urlencode($proto . $_SERVER['HTTP_HOST'] . '/wp-content/themes/turismovalencia/crea-tu-blog.php' ));
